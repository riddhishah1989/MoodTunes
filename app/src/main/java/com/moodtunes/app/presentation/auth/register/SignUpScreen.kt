...
fun SignUpScreen(onNavigationLoginScreen: () -> Unit, onSignUpSuccess: () -> Unit) {
    ...
    RegisterContent(goBackToLogin = onNavigationLoginScreen, onSignUpClick = {
        onSignUpSuccess()
    }, selectedGenreIds)
}
...
fun RegisterContent(
    ...
    Column(
        ...
        Spacer(modifier = Modifier.size(15.dp))
        GenreChipGroup(StaticData.genreList, selectedGenreIds, onGenreToggle = {})
        Spacer(modifier = Modifier.size(15.dp))
        PrimaryButton(text = "Register", onClick = onSignUpClick)
...
fun PreviewRegister() {
    RegisterContent(goBackToLogin = {}, onSignUpClick = {}, selectedGenreIds = emptyList())
}
